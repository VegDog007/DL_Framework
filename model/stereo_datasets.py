...
class SceneFlowDatasets(StereoDataset):
    def __init__(self, aug_params=None, root='/group/project79/dataset/seneflow/', dstype='frames_finalpass', things_test=False):
    ...
    def _add_things_sub(self, split='TRAIN'):
        """ Add FlyingThings_sub 3D data """

        original_length = len(self.disparity_list)
        # root = osp.join(self.root, 'FlyingThings3D')
        root = self.root
        left_images = sorted( glob(osp.join(root, 'Flying3d_sub/FlyingThings3D_subset/train/image_clean/left/*.png')) )
        right_images = [ im.replace('left', 'right') for im in left_images ]
        disparity_images = [ im.replace("image_clean", 'disparity').replace('.png', '.pfm') for im in left_images ]
    ...
    def _add_monkaa(self, split="TRAIN"):
        ...
        original_length = len(self.disparity_list)
        root = self.root
        left_images = sorted( glob(osp.join(root, 'monkaa/', self.dstype, '*/left/*.png')) )
    ...
    def _add_driving(self, split="TRAIN"):
        ...
        original_length = len(self.disparity_list)
        root = self.root
        left_images = sorted( glob(osp.join(root, 'driving' ,self.dstype, '*/*/*/left/*.png')) )
        right_images = [ image_file.replace('left', 'right') for image_file in left_images ]
        disparity_images = [ im.replace(self.dstype, 'disparity').replace('.png', '.pfm') for im in left_images ]
...
class KITTI(StereoDataset):
    def __init__(self, aug_params=None, root='/group/project79/dataset/KITTI2012', image_set='training'):
        super(KITTI, self).__init__(aug_params, sparse=True, reader=frame_utils.readDispKITTI)
        assert os.path.exists(root)
        if (image_set=="training"):
            root_12 = '/group/project79/dataset/KITTI2012'
            ...
            disp_list = sorted(glob(os.path.join(root_12, image_set, 'disp_occ/*_10.png'))) #if image_set == 'training' else [osp.join(root, 'training/disp_occ/000085_10.png')]*len(image1_list)

            root_15 = '/group/project79/dataset/KITTI2015'
            ...
            disp_list += sorted(glob(os.path.join(root_15,image_set, 'disp_occ_0/*_10.png'))) #if image_set == 'training' else [osp.join(root, 'training/disp_occ_0/000085_10.png')]*len(image1_list)
        elif(image_set=="val"):
            root_15 = '/group/project79/dataset/KITTI2015'
            image1_list = sorted(glob(os.path.join(root_15, image_set, 'image_2/*_10.png')))
            image2_list = sorted(glob(os.path.join(root_15, image_set, 'image_3/*_10.png')))
